...
public class JanelaCadastroProfessor extends JInternalFrame implements ActionListener {
	...
	private JButton butaoCancelarProfessor = new JButton("Cancelar");
	private JButton botaoAdicionarProfessor = new JButton("Adicionar");
	...
	public JanelaCadastroProfessor() {
		...
		botaoAdicionarProfessor.setBounds(433, 325, 97, 25);
		getContentPane().add(botaoAdicionarProfessor);
		botaoAdicionarProfessor.addActionListener(this);
		
		butaoCancelarProfessor.setBounds(542, 325, 97, 25);
		butaoCancelarProfessor.addActionListener(this);
		getContentPane().add(butaoCancelarProfessor);
	...
	public void actionPerformed(ActionEvent evento) {
		if(evento.getSource() == botaoAdicionarProfessor){
...
//			JComboBox comboBoxSexo = new JComboBox(sexos);
			...
			if(nome.length() == 0){
				...
				professor.setCref(numeroIndetificacao);
				professor.setEndereco(new Endereco(cidade, estado, rua, numeroDaCasa));
				professor.getContato().setEmail(email);
				professor.getContato().setTelefone(contato);
				professor.setDataDeNascimento(dataDeNascimento);
				Sistema.getGerenteProfessor().cadastrarProfessor(professor);
				JOptionPane.showMessageDialog(null, "Cadastrado com sucesso");
				int opcao = JOptionPane.showConfirmDialog(null, "Deseja cadastrar outro Professors?");
				switch (opcao){
					case 0:
						setarTextFilds();
						break;
		...
		if(evento.getSource() == butaoCancelarProfessor){
			setVisible(false);
		}
	}
